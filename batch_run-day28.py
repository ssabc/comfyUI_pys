...
def patch_prompt(prompt, args, run_index, current_image_path):
    ...
    if args.save_node:
        ...
        save_inputs[args.output_input] = full_output_dir
        save_inputs[args.filename_prefix_input] = image_path.stem
        save_inputs["extension"] = output_extension_for(image_path)
...
ALL_IMAGE_EXTS = tuple(sorted({ext for exts in FILETYPE_EXTS.values() for ext in exts}))


def exts_for(filetype):
    if not filetype or filetype == "all":
        return ALL_IMAGE_EXTS
...
# Match SequentialImageLoader.list_images ordering exactly.
def scan_all_images(folder, include_subfolders=False, filetype="all", reverse=False):
    folder = (folder or "").strip().strip('"').strip("'")
    if not folder or not os.path.isdir(folder):
        return []

    image_extensions = exts_for(filetype)
    file_list = []
    if include_subfolders:
        ...
        file_list.sort(key=lambda p: natural_key(os.path.relpath(p, folder)))
    else:
        for f in os.listdir(folder):
            path = os.path.join(folder, f)
            if os.path.isfile(path) and f.lower().endswith(image_extensions):
...
def main():
    parser = argparse.ArgumentParser(description="Run Day28 image batch workflow")
    ...
    scan_workflow = copy.deepcopy(workflow)
    for assignment in args.set:
        apply_set(scan_workflow, assignment)
    loader_inputs = scan_workflow.get(args.loader_node, {}).get("inputs", {})
    reverse = truthy(loader_inputs.get("reverse", False))

    img_list = scan_all_images(args.input, args.include_subfolders, args.filetype, reverse)
    total_runs = len(img_list)
    print(f"Detected {total_runs} images. Starting batch run...")
    ...
    with log_path.open("w", newline="", encoding="utf-8-sig") as f:
        ...
        for run_index in range(total_runs):
            prompt_id = ""
            try:
                current_image_path = img_list[(args.start + run_index) % total_runs]
                prompt = patch_prompt(workflow, args, run_index, current_image_path)
    ...
    print(f"Batch run completed. Log file: {log_path}")
